                            ...
                            <div class="row">
                                <h5 class="card-title mb-0 col-sm-8 col-md-10">
                                    ...
                                    @if ($id != 0 && $parent_id !== null)
                                        <a href="{{ route('category.index', $parent_id) }}"> / Back</a>
                                    @endif
                                </h5>
                                ...
                                <div class="hstack flex-wrap gap-2   mb-lg-0 mb-0 col-sm-2 col-md-1">
                                    @if ($num_of_parents < 3)
                                        <a href="{{ route('category.addCategory', $id) }}"
...
@push('js')
    <script>
        var table = $('#alternative-pagination').DataTable({
            ajax: '{{ route('category.dataTable', $id) }}',
            columns: [{
                    'data': null,
...

                {
                    ...
                    render: function(data) {
                        ...
                        var num = {{ $num_of_parents }};
                        var editUrl = '{{ route('category.edit', ':id') }}';
                        var deleteUrl = '{{ route('category.archive', ':id') }}';
                        var sub = '{{ route('category.index', ':id') }}';

                        editUrl = editUrl.replace(':id', data.id);
                        deleteUrl = deleteUrl.replace(':id', data.id);
                        sub = sub.replace(':id', data.id);
                        ...
                        var subButton = '<a href="' + sub +
                            '"> <i class="bx bx-show btn btn-success"></i></a>';

                        if (num >= 2) {
                            return editButton + '' + deleteButton;
                        } else {
                            return editButton + ' ' + subButton + '' + deleteButton;
                        }
