                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content border-0 overflow-hidden">
                                            ...
                                            <div class="modal-body">
                                                <form action="{{ route('shipping.storeShippingTime') }}" method="POST">
                                                    @csrf
                                                    <div class="col-md-12">
                                                        <div class="mb-1" id="subsubcategories-container">
                                                            <h6 class="fw-semibold">Add New Shipping Time</h6>

                                                            <div class="col-md-12">
                                                                <select onchange="fetchAreas(this.value)" name="governorates" class="form-control type-select mb-1">
                                                                    <option disabled selected value="">Select Governorate</option>
                                                                    @foreach ($governorates as $value)
                                                                        <option value="{{ $value->id }}">
                                                                            {{ $value->title_ar }} / {{ $value->title_en }}
                                                                        </option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            ...
                                                            <div class="col-md-12 mt-1">
                                                                <input class="form-control" placeholder="{{ App::getLocale() == 'ar' ? 'ادخل الوقت المقرب للتوصيل' : 'Enter The Shipping Time' }}" name="shipping_time" type="text">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="text-end">
...
@push('js')
    <script>
        var table = $('#alternative-pagination').DataTable({
            ajax: '{{ route('shipping.timeDataTable') }}',
            columns: [{
                    'data': null,
...

                {
                    ...
                    render: function(data) {
                        ...
                        var title =
                            '{{ App::getLocale() == 'ar' ? 'governorate_name_ar' : 'governorate_name_en' }}';
                        return data[title];
                    }
                },
                {
                    ...
                    render: function(data) {
                        // Choose the appropriate name based on the selected language
                        var title = '{{ App::getLocale() == 'ar' ? 'area_name_ar' : 'area_name_en' }}';
                        return data[title];
                    }
                },
    ...
    <script>
        $(document).on('click', '#status', function() {
            var url = '{{ route('shipping.timeStatus', ':id') }}';
...
<script>
    function fetchAreas(governorateId) {
        if (governorateId) {
            $.ajax({
                url: "{{ route('shipping.getSubTypes', ':id') }}".replace(':id', governorateId),
                type: "GET",
                dataType: "json",
                success: function(data) {
                    $('select[name="areas"]').empty();
                    $('select[name="areas"]').append('<option disabled selected value="">Select Area</option>');
                    $.each(data, function(key, value) {
                        $('select[name="areas"]').append('<option value="'+ value.id +'">'+ value.area_ar + ' / ' + value.area_en +'</option>');
                    });
                }
            });
        } else {
