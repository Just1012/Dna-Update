                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <div class="row">
                                <h5 class="card-title mb-0 col-sm-8 col-md-10">{{ __('list') }} {{ $page_title }} </h5>
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            var table = $('#alternative-pagination').DataTable({
                ajax: '{{ route('admins.dataTable', $type) }}',
                columns: [{
                        'data': null,
...

                    {
                        ...
                        render: function(data) {
                            ...
                            if (data.id == 1) {
                                var editButton =
                                    '<button class="btn btn-warning" id="admin-edit" data-id="' +
                                    data.id + '"><i class="bx bxs-edit"></i></button>';
                            } else {
    ...
    <script>
        $(document).on('click', '#admin-edit', function(e) {
            ...
            if (userId == 1) {
                toastr.error('Cannot update the admin.', 'Error!');
                e.preventDefault(); // Prevent the default action
                return; // Exit the function if the user ID is 1
            }
        });
        $(document).on('click', '#status', function(e) {
            ...
            if (userId == 1) {
                toastr.error('Cannot deactivate the admin.', 'Error!');
                e.preventDefault(); // Prevent the default action to stop the checkbox from toggling
                return; // Exit the function if the user ID is 1
            }
